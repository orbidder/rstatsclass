...
dis_mod <- nls(NSD ~ d/(1+exp((th-t)/p1)), 
               start = list(d=5000,
                            th=60,
                            p1=10))
...
#First we will convert the data in to adehabitat's ltraj object type
xy1 <- as.data.frame(cbind("x" = df_elk2$UTM_X, "y" = df_elk2$UTM_Y)) #needs to be a dataframe
date1 <- df_elk2$Date_Time #needs to be POSIXct class, which we made it in to above
id1 <- paste(df_elk2$id,df_elk2$Year,sep = "_") #combine to make a new id, which is the animal_year format
