...
coords = matrix(c(elk_utm@bbox[1], elk_utm@bbox[2],
                ...
                ncol = 2, byrow = TRUE)
P1 = Polygon(coords)
Ps1 = SpatialPolygons(list(Polygons(list(P1), ID = "a")), proj4string=CRS("+proj=utm +zone=12 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0"))
plot(Ps1)

buffer <- gBuffer(Ps1, width = 4000)
plot(buffer)
